...
#' as 0 and 1.
#'
#' For **logical** input, `TRUE` is mapped to 1 and `FALSE`
#' to 0.
#'
#' For **numeric** input, values must already be 0 or 1 (or `NA`);
#' any other value raises an error.
#'
#' For **factor** input, the vector must have exactly two levels. By
#' default the second level is coded as 1. Use `pos` to specify which
...
#'   `pos` are coded as 1 and all others as 0. Must be one of the
...
#'   `pos`, indicating which value is coded as 1.
#'
#' @return a numeric vector of 0s and 1s (and `NA`s where present in
#'   `x`). For factor and character input, the result carries a
#'   `"coding"` attribute, a named integer vector documenting which
