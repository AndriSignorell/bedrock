...
#'
#' Compute the prime factorization(s) of integer(s) `n`, using
#' Pollard's rho algorithm with deterministic Miller-Rabin primality
#' testing (64-bit, implemented in C++).
#'
#' `n` must not exceed `2^53` (`9007199254740992`), the
...
#' but not all of them are, and above this bound `n` may already have
...
#' `gmp::factorize()`, which represents arbitrarily large integers
#' exactly (e.g. via `gmp::as.bigz()` or a string).
#'
#' @param n vector of positive whole numbers to factorize, not exceeding
#'   `2^53`.
#'
#' @return a named [list()] of the same length as `n`, each
#' element a 2-column matrix with column `"p"` the prime factors in
#' increasing order and column `"m"` their respective exponents (or
#' multiplicities), i.e., for a prime number `n`, the resulting matrix
#' is `cbind(p = n, m = 1)`.
#'
#' Each prime appears in exactly one row, so `prod(p^m)` returns
#' `n` and `p` is strictly increasing. `n = 1` yields a
#' matrix with zero rows: 1 is the empty product, and
#' `prod(numeric(0))` is 1 accordingly.
#'
#' @examples
#'
#' factorize(47)
