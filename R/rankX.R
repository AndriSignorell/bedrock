...
#'   provided, they are ranked lexicographically (like `order`).
#'   All inputs must have the same length.
#' @param decreasing logical; if `TRUE`, larger values receive smaller
...
#'   input.
#' @param na.last logical or `"keep"`; determines the placement of
#'   `NA` values. Passed to `data.table::frankv`.
#' @param ties.method character string specifying how ties are handled.
#'   One of:
#'   \describe{
#'     \item{`"average"`}{average of the ranks for tied values (default).}
#'     \item{`"first"`}{ranks assigned in order of appearance.}
#'     \item{`"last"`}{ranks assigned in reverse order of appearance.}
#'     \item{`"random"`}{ranks assigned at random.}
#'     \item{`"max"`}{maximum rank for tied values.}
#'     \item{`"min"`}{minimum rank for tied values.}
#'     \item{`"dense"`}{like `"min"`, but ranks are consecutive
...
#' \itemize{
#'   \item Supporting dense ranking (`ties.method = "dense"`)
