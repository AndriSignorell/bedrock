...
test_that("appendX.matrix inserts multiple rows in row-major order", {
  x <- matrix(1:6, nrow = 3)

  res <- appendX(x, 10:13, rows = TRUE)
  expect_equal(nrow(res), 5)
  expect_equal(as.numeric(res[4, ]), c(10, 11))
  expect_equal(as.numeric(res[5, ]), c(12, 13))
})
...
test_that("appendX.TOne preserves class and attributes", {
  ...
  expect_equal(nrow(unclass(res)), 4)
})
...
test_that("single value insertion works", {
  ...
  expect_no_warning(res <- appendX(x, 99))
