...
test_that("returns closest value when no exact match", {
  expect_equal(closest(c(1, 2, 4, 5), 3), c(2, 4))
})
...
test_that('output = "index" returns position', {
  expect_equal(
    closest(c(10, 20, 30), 20, output = "index"),
    2L
  )
})


test_that('output = "index" returns all tied positions', {
  
  x <- c(5, 5, 9)
  
  res <- closest(x, 5, output = "index")
  
  expect_equal(sort(res), c(1L, 2L))
})


test_that('output = "index" returns integer vector', {
  
  res <- closest(1:5, 3, output = "index")
...
test_that("na.rm = FALSE returns NA when x is all-NA", {
  ...
  expect_true(
    all(is.na(closest(c(NA_real_, NA_real_), 5)))
  )
})
...
test_that("na.rm = TRUE with all-NA returns NA", {
  ...
  expect_true(
    is.na(
      closest(c(NA_real_, NA_real_), 5, na.rm = TRUE)
    )
  )
})
...
test_that("empty x returns NA", {
  ...
  expect_true(
    is.na(closest(numeric(0), 5))
  )
})


test_that('empty x with output = "index" returns NA', {
  
  expect_true(
    is.na(
      closest(numeric(0), 5, output = "index")
    )
  )
})
...
test_that("vectorized output recycled over a", {
...
test_that("output vectorized over scalar a returns list", {
...
test_that("invalid output raises error", {
  
  expect_error(
    closest(1:5, 3, output = "foo")
  )
})
